const state = {
  ...
  selectedWhatsappAuditReview: "",
  whatsappAuditItems: [],
  whatsappAuditVisibleCount: 24,
...
function buildQuery() {
  ...
  if (state.activeView === "whatsappAuditView") params.set("include_whatsapp_audit", "1");
...
function render() {
  ...
  if (state.activeView === "whatsappAuditView") {
    renderWhatsappAudit(report.whatsapp_audit || {});
  }
...
function callerSelect(name, placeholder, selected = "") {
  return `
    <select name="${escapeHtml(name)}" required>
      <option value="">${escapeHtml(placeholder)}</option>
      ${FOLLOWUP_CALLERS.map(person => `<option value="${escapeHtml(person)}" ${selected === person ? "selected" : ""}>${escapeHtml(person)}</option>`).join("")}
    </select>
  `;
}
...
function renderWhatsappAudit(audit) {
  const list = document.getElementById("whatsappAuditList");
  if (!list) return;

  if (!audit.items) {
    list.innerHTML = `<div class="emptyState">Carregando avaliação de WhatsApp...</div>`;
    return;
  }

  const totals = audit.totals || {};
  state.whatsappAuditItems = audit.items || [];
  const reference = document.getElementById("whatsappAuditReferenceDate");
  if (reference) {
    reference.textContent = `Base: ${formatFullDay(audit.date_from || state.dateFrom)} a ${formatFullDay(audit.date_to || state.dateTo)}`;
  }
  document.getElementById("whatsappAuditTotal").textContent = integerFormat(totals.total || 0);
  document.getElementById("whatsappAuditReviewed").textContent = integerFormat(totals.reviewed || 0);
  document.getElementById("whatsappAuditAverage").textContent = totals.average_score == null ? "-" : integerFormat(totals.average_score);
  document.getElementById("whatsappAuditCritical").textContent = integerFormat(totals.critical || 0);

  const filter = document.getElementById("whatsappAuditReviewFilter");
  if (filter) filter.value = state.selectedWhatsappAuditReview;

  const notice = document.getElementById("whatsappAuditMessageNotice");
  if (notice) {
    notice.textContent = audit.messages_available
      ? "Listando somente atendimentos em que o Kommo liberou texto de conversa. A IA avalia apenas esses registros."
      : "O Kommo ainda não enviou texto de mensagens nesta base. A análise automática depende de liberar ou sincronizar as mensagens externas.";
  }

  renderWhatsappAuditRubric(audit.rubric || WHATSAPP_AUDIT_RUBRIC);
  renderWhatsappAuditList();
}

function renderWhatsappAuditRubric(rubric) {
  const container = document.getElementById("whatsappAuditRubric");
  if (!container) return;
  container.innerHTML = (rubric || WHATSAPP_AUDIT_RUBRIC).map(group => {
    const title = group.title || group.label || "";
    const points = group.points ?? -Math.abs(Number(group.penalty || 0));
    return `
    <article class="auditRubricGroup ${escapeHtml(group.id || "")}">
      <h3>${escapeHtml(title)}</h3>
      <strong>${points < 0 ? "-" : ""}${Math.abs(points || 0)} pontos</strong>
      <ul>
        ${(group.items || [])
          .filter(item => !DISABLED_WHATSAPP_AUDIT_FAILURES.has(item.label || item))
          .map(item => `<li>${escapeHtml(item.label || item)}</li>`).join("")}
      </ul>
    </article>
  `;
  }).join("");
...
function filteredWhatsappAuditItems() {
  const filter = state.selectedWhatsappAuditReview;
  return (state.whatsappAuditItems || []).filter(item => {
    if (!filter) return true;
    if (filter === "reviewed") return !!item.reviewed;
    if (filter === "pending") return !item.reviewed;
    if (filter === "critical") return (item.severity || "") === "critical" || Number(item.score || 100) <= 0;
    return true;
  });
}

function whatsappAuditItemKey(item) {
  return item?.audit_key || `${item?.clinic || state.selectedClinic || ""}:${item?.lead_id || ""}`;
}

function whatsappAuditItemByKey(key) {
  return (state.whatsappAuditItems || []).find(item => whatsappAuditItemKey(item) === key);
}

function auditSeverityLabel(severity, score) {
  if (severity === "critical" || Number(score || 100) <= 0) return "Crítico";
  if (severity === "grave") return "Grave";
  if (severity === "moderate") return "Atenção";
  if (severity === "clean" || severity === "ok") return "Sem desvios";
  return "Pendente";
}

function whatsappAuditFailureLabel(item) {
  if (typeof item === "string") return item;
  return item?.label || item?.id || "";
}

function renderWhatsappAuditList() {
  const container = document.getElementById("whatsappAuditList");
  if (!container) return;

  const items = filteredWhatsappAuditItems();
  const visible = items.slice(0, state.whatsappAuditVisibleCount);
  const count = document.getElementById("whatsappAuditFilteredCount");
  if (count) count.textContent = `${integerFormat(items.length)} atendimentos no filtro`;

  if (!items.length) {
    container.innerHTML = `<div class="emptyState">Nenhum atendimento com texto de conversa encontrado para os filtros atuais.</div>`;
    return;
  }

  const rubricItems = WHATSAPP_AUDIT_RUBRIC.flatMap(group =>
    group.items.filter(item => !DISABLED_WHATSAPP_AUDIT_FAILURES.has(item)).map(item => {
      const points = group.points ?? -Math.abs(Number(group.penalty || 0));
      return {
      id: whatsappAuditFailureLabel(item),
      label: whatsappAuditFailureLabel(item),
      group: group.title || group.label || "",
      points,
      };
    })
  );
  ...
  container.innerHTML = visible.map(item => {
    const key = whatsappAuditItemKey(item);
    const failures = new Set((item.failures || []).filter(failure => !DISABLED_WHATSAPP_AUDIT_FAILURES.has(failure)));
    const score = item.score == null ? null : Number(item.score);
    const severity = item.severity || (score == null ? "pending" : (score <= 0 ? "critical" : "clean"));
    const isClean = severity === "clean" || severity === "ok";
    const samples = (item.message_samples || [])
      .map(message => typeof message === "string" ? {author: "Mensagem", text: message} : message)
      .filter(message => message && message.text);
    const actionUrl = item.kommo_lead_url || item.kommo_search_url || "";
    const actionLabel = item.kommo_lead_url ? "Abrir lead no Kommo" : "Buscar contato no Kommo";
    const aiSummary = item.ai_summary || (item.ai_generated ? item.notes : "");
    const aiRecommendation = item.ai_recommendation || "";

    return `
      <article class="followupCard whatsappAuditCard ${escapeHtml(severity)}">
        <div class="auditCardTop">
          <div>
            <span class="followupTag ${severity === "critical" ? "red" : isClean ? "active" : "monitor"}">${escapeHtml(auditSeverityLabel(severity, score))}</span>
            <h3>${escapeHtml(item.patient_name || item.lead_name || "Contato sem nome")}</h3>
            <p>${escapeHtml(item.pipeline_name || "Funil não identificado")} · ${escapeHtml(item.responsible_name || "Responsável não identificado")}</p>
          </div>
          <strong class="auditScore">${score == null ? "-" : integerFormat(score)}<span>pontos</span></strong>
        </div>
        <div class="auditMetaGrid">
          <span><b>Última interação</b>${escapeHtml(formatFullDay(item.updated_at_date || item.last_message_date) || "-")}</span>
          <span><b>Paciente</b>${escapeHtml(item.patient_name || "Não cruzado")}</span>
          <span><b>Telefone</b>${escapeHtml(item.patient_phone || item.lead_phone || "-")}</span>
          <span><b>Avaliado por</b>${escapeHtml(item.evaluator || "-")}</span>
        </div>
        <div class="auditActions">
          ${actionUrl ? `<a class="auditActionButton" href="${escapeHtml(actionUrl)}" target="_blank" rel="noopener">${escapeHtml(actionLabel)}</a>` : ""}
          <button class="auditActionButton primary" type="button" data-whatsapp-ai="${escapeHtml(key)}">${item.ai_generated ? "Reavaliar com IA" : "Avaliar com IA"}</button>
        </div>
        <div class="auditPreview">
          ${samples.length ? `
            <ul>
              ${samples.slice(0, 3).map(message => `
                <li><b>${escapeHtml(message.author || "Mensagem")}</b><span>${escapeHtml(message.text)}</span></li>
              `).join("")}
            </ul>
          ` : "Sem texto de mensagens sincronizado para avaliação automática."}
        </div>
        ${aiSummary || aiRecommendation ? `
          <div class="auditAiSummary">
            <strong>Análise da IA</strong>
            ${aiSummary ? `<p>${escapeHtml(aiSummary)}</p>` : ""}
            ${aiRecommendation ? `<small>${escapeHtml(aiRecommendation)}</small>` : ""}
          </div>
        ` : ""}
        <form class="auditReviewForm" data-audit-key="${escapeHtml(key)}">
          <div class="auditFailureGrid">
            ${rubricItems.map(failure => `
              <label class="auditFailureOption">
                <input type="checkbox" name="failures" value="${escapeHtml(failure.id)}" ${failures.has(failure.id) ? "checked" : ""}>
                <span>${escapeHtml(failure.label)} <small>${failure.points} pts</small></span>
              </label>
            `).join("")}
          </div>
          <div class="auditFormFooter">
            ${callerSelect("evaluator", "Quem avaliou", item.evaluator || "")}
            <textarea name="notes" rows="2" placeholder="Observações da avaliação">${escapeHtml(item.notes || "")}</textarea>
...
async function saveWhatsappAuditReview(event) {
  event.preventDefault();
  const form = event.target;
  const item = whatsappAuditItemByKey(form.dataset.auditKey);
  if (!item) return;
  const button = form.querySelector("button");
  const original = button.textContent;
  button.disabled = true;
  ...
  try {
    ...
    if (state.selectedClinic) params.set("clinic", state.selectedClinic);
    const payload = {
      audit_key: whatsappAuditItemKey(item),
      clinic: state.selectedClinic,
      lead_id: item.lead_id,
      lead_name: item.lead_name,
      patient_name: item.patient_name,
      patient_phone: item.patient_phone || item.lead_phone,
      pipeline_id: item.pipeline_id,
      pipeline_name: item.pipeline_name,
      responsible_user_id: item.responsible_user_id,
      responsible_name: item.responsible_name,
      failures: Array.from(form.querySelectorAll("input[name='failures']:checked")).map(input => input.value),
      notes: form.elements.notes?.value || "",
      evaluator: form.elements.evaluator?.value || "",
    };
    const res = await fetch(`/api/whatsapp-audit-review?${params.toString()}`, {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify(payload),
    });
    const result = await res.json();
    if (!res.ok || !result.ok) throw new Error(result.error || "Não foi possível salvar a avaliação.");
    Object.assign(item, result.review || {});
    showNotice("Avaliação salva.");
    renderWhatsappAuditList();
  } catch (error) {
    showNotice(error.message || "Não foi possível salvar a avaliação.");
  } finally {
    button.disabled = false;
    button.textContent = original;
  }
...
async function evaluateWhatsappAuditWithAi(button) {
  const item = whatsappAuditItemByKey(button.dataset.whatsappAi);
  if (!item) return;
  const original = button.textContent;
  button.disabled = true;
  ...
  try {
    ...
    const res = await fetch(`/api/whatsapp-audit-ai?${params.toString()}`, {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify(payload),
    });
    const result = await res.json();
    if (!res.ok || !result.ok) throw new Error(result.error || "Não foi possível avaliar com IA.");
    Object.assign(item, result.review || {});
    showNotice("IA avaliou o atendimento e salvou a nota.");
    renderWhatsappAuditList();
  } catch (error) {
    showNotice(error.message || "Não foi possível avaliar com IA.");
  } finally {
    button.disabled = false;
    button.textContent = original;
  }
...
document.getElementById("whatsappAuditReviewFilter")?.addEventListener("change", event => {
  state.selectedWhatsappAuditReview = event.target.value;
  state.whatsappAuditVisibleCount = 24;
  renderWhatsappAuditList();
});
document.getElementById("whatsappAuditList")?.addEventListener("submit", event => {
  if (event.target.matches(".auditReviewForm")) saveWhatsappAuditReview(event);
});
document.getElementById("whatsappAuditList")?.addEventListener("click", event => {
  const aiButton = event.target.closest("[data-whatsapp-ai]");
  if (aiButton) {
    evaluateWhatsappAuditWithAi(aiButton);
    return;
  }
  if (event.target.closest("[data-whatsapp-audit-more]")) {
    state.whatsappAuditVisibleCount += 24;
    renderWhatsappAuditList();
  }
});
