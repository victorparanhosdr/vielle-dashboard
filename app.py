...
def init_db():
    with db() as conn:
        conn.executescript(
            ...
            create table if not exists whatsapp_audit_reviews (
            ...
            create index if not exists idx_whatsapp_audit_reviews_clinic on whatsapp_audit_reviews(clinic);
            create index if not exists idx_whatsapp_audit_reviews_lead on whatsapp_audit_reviews(clinic, lead_id);
        ...
        for statement in (
            ...
            "alter table whatsapp_audit_reviews add column ai_payload_json text",
        ):
            try:
...
def report_data(pipeline_ids=None, date_from=None, date_to=None, doctor=None, seller=None, include_followup=False, include_quote_followup=False, include_whatsapp_audit=False):
    ...
    with db() as conn:
        ...
        whatsapp_audit = None
        if include_whatsapp_audit:
            whatsapp_audit = build_whatsapp_audit(
                conn,
                effective_pipeline_ids,
                start_ts,
                end_ts,
...
WHATSAPP_AUDIT_RUBRIC = [
    {
        "severity": "critical",
        "label": "Falha gravíssima",
        "penalty": 100,
        "items": [
            "Follow-up Consistente",
            "Ortografia e Profissionalismo",
        ],
    },
    {
        "severity": "grave",
        ...
        "items": [
            "Confidencialidade",
            "Cordialidade",
            "Empatia em Reclamações",
            "Conhecimento do Produto/Serviço",
            "Resolução de Problemas",
            "Gestão de Expectativas",
            "Pontualidade no Follow-Up",
        ],
    },
    {
        "severity": "moderate",
        "label": "Falha moderada",
        "penalty": 30,
        "items": [
            "Informação Completa",
            "Simpatia e Tom Amigável",
            "Coerência e Clareza",
            "Uso de Scripts Padrões",
...
DISABLED_WHATSAPP_AUDIT_FAILURES = {
    "Prontidão no Atendimento",
}


def whatsapp_audit_failure_catalog():
    catalog = {}
    for group in WHATSAPP_AUDIT_RUBRIC:
        for item in group["items"]:
            catalog[item] = {
                "severity": group["severity"],
                "label": group["label"],
                "penalty": int(group["penalty"]),
            }
    return catalog


def normalize_whatsapp_audit_failures(failures):
    if not isinstance(failures, list):
        failures = []
    catalog = whatsapp_audit_failure_catalog()
    normalized = []
    seen = set()
    for value in failures:
        name = str(value or "").strip()
        if name in DISABLED_WHATSAPP_AUDIT_FAILURES:
            continue
...
def whatsapp_audit_score(failures):
    catalog = whatsapp_audit_failure_catalog()
    penalty = sum(catalog[name]["penalty"] for name in failures if name in catalog)
...
def build_clinica_patient_lookup(conn):
    lookup = {}
    rows = conn.execute(
        "select uuid, name, phone, email, raw_json from clinica_patients"
    ).fetchall()
    for row in rows:
        patient = dict(row)
        for key in phone_lookup_keys(row["phone"] or ""):
            lookup.setdefault(("phone", key), patient)
        name_key = normalize_lookup_text(row["name"] or "")
        if name_key:
            lookup.setdefault(("name", name_key), patient)
        payload = safe_json_object(row["raw_json"])
...
def extract_lead_contact_info(raw_json):
    payload = safe_json_object(raw_json)
    phones = []
    emails = []
    names = []
    for value in iter_string_values(payload):
        ...
        if not text:
            continue
        if "@" in text and "." in text and len(text) <= 160:
            emails.append(text)
        for key in phone_lookup_keys(text):
            phones.append(key)
    for field in payload.get("custom_fields_values") or []:
        ...
        for value in field.get("values") or []:
            ...
            if not field_value:
                continue
            if "nome" in field_name or "paciente" in field_name or "contato" in field_name:
                names.append(field_value)
    return {
        "phones": sorted(set(phones), key=len, reverse=True),
        "emails": sorted(set(emails)),
        "names": sorted(set(names)),
    }


def find_patient_for_whatsapp_audit(lookup, lead_name, contact_info):
    for phone in contact_info.get("phones") or []:
        patient = lookup.get(("phone", phone))
        if patient:
            return patient
    for name in [lead_name, *(contact_info.get("names") or [])]:
        patient = lookup.get(("name", normalize_lookup_text(name)))
        if patient:
            return patient
    return None
...
def is_probable_message_text(text):
    text = str(text or "").strip()
    if not text:
        return False
    if len(text) > 4000:
        return False
    if len(re.sub(r"[^A-Za-zÀ-ÿ]", "", text)) < 2:
        return False
    if re.fullmatch(r"[\d\s()+\-./]+", text):
        return False
    if re.fullmatch(r"https?://\S+", text, flags=re.I):
        return False
    if re.fullmatch(r"[A-Za-z0-9_\-]{24,}", text) and " " not in text:
        return False
...
def is_probable_message_event(event_type, raw_payload):
    event_type_text = str(event_type or "").lower()
    if any(hint in event_type_text for hint in MESSAGE_EVENT_TYPE_HINTS):
        return True
    if any(hint in event_type_text for hint in NON_MESSAGE_EVENT_TYPE_HINTS):
        return False
    payload_text = json.dumps(raw_payload or {}, ensure_ascii=False).lower()
    return any(hint in payload_text for hint in ("whatsapp", "chat", "message", "mensagem"))


def collect_message_texts(value, output, max_count=3, max_chars=280):
    if len(output) >= max_count:
        return
    if isinstance(value, dict):
        for key, nested in value.items():
            lowered = str(key or "").lower()
            if lowered in MESSAGE_TEXT_KEYS:
                text = str(nested or "").strip()
                if is_probable_message_text(text):
                    output.append(text[:max_chars])
                    if len(output) >= max_count:
                        return
            collect_message_texts(nested, output, max_count=max_count, max_chars=max_chars)
    elif isinstance(value, list):
        for nested in value:
            collect_message_texts(nested, output, max_count=max_count, max_chars=max_chars)
            if len(output) >= max_count:
...
def lead_message_samples(conn, lead_id):
    samples = []
    rows = conn.execute(
        """
        select type, raw_json
        from lead_interaction_events
        where lead_id = ?
    ...
    for row in rows:
        raw_payload = safe_json_object(row["raw_json"])
        if not is_probable_message_event(row["type"], raw_payload):
            continue
        collect_message_texts(raw_payload, samples)
        if len(samples) >= 3:
            break
    unique = []
    seen = set()
...
def lead_message_transcript(conn, lead_id, limit=None):
    query = """
        select created_at, type, raw_json
        from lead_interaction_events
        where lead_id = ?
    ...
    for row in rows:
        ...
        if not is_probable_message_event(row["type"], raw_payload):
            continue
        texts = []
        collect_message_texts(raw_payload, texts, max_count=20, max_chars=1600)
        for text in texts:
            ...
            seen.add(key)
            transcript.append({
                "data": timestamp_label(row["created_at"]) if row["created_at"] else "",
                "tipo": row["type"] or "",
                "texto": text,
            })
    return transcript
...
def compact_transcript_for_ai(transcript, max_chars=90000):
    if not transcript:
        return [], False
    ...
    for item in reversed(transcript[40:]):
        item_size = len(json.dumps(item, ensure_ascii=False))
        if used + item_size + 300 > max_chars:
            break
        tail.append(item)
        used += item_size
    tail.reverse()
    omitted = max(len(transcript) - len(head) - len(tail), 0)
    marker = {
        "data": "",
        "tipo": "sistema",
        "texto": f"... {omitted} mensagens intermediárias omitidas por limite técnico da IA ...",
    }
...
def openai_chat_json(messages):
    api_key = str(config_value("OPENAI_API_KEY", "") or "").strip()
    if not configured(api_key):
        raise ValueError("OPENAI_API_KEY não configurada. Configure a chave da OpenAI em Configurações ou no Railway para a IA avaliar automaticamente.")
    model = str(config_value("OPENAI_MODEL", "gpt-4.1-mini") or "gpt-4.1-mini").strip()
    payload = {
        "model": model,
        "messages": messages,
        "temperature": 0.1,
    ...
    try:
        ...
        detail = exc.read().decode("utf-8", errors="replace")
        raise ValueError(f"OpenAI retornou {exc.code}: {detail[:500]}")
    content = (((data.get("choices") or [{}])[0].get("message") or {}).get("content") or "").strip()
    if not content:
        raise ValueError("A IA não retornou uma avaliação.")
    try:
        return parse_json_response_text(content)
    except (TypeError, ValueError, json.JSONDecodeError) as exc:
        raise ValueError(f"A IA retornou uma resposta fora do formato esperado: {exc}")


def whatsapp_audit_review_payload(payload, score, severity, failures, reviewed_at, ai_generated=False, ai_summary="", ai_recommendation="", notes=None):
    return {
        "audit_key": str(payload.get("audit_key") or "").strip(),
        "clinic": current_clinic_id(),
        "lead_id": int(payload.get("lead_id") or 0) or None,
        "lead_name": str(payload.get("lead_name") or "").strip(),
        "patient_name": str(payload.get("patient_name") or "").strip(),
        "patient_phone": str(payload.get("patient_phone") or "").strip(),
        "pipeline_id": int(payload.get("pipeline_id") or 0) or None,
        "pipeline_name": str(payload.get("pipeline_name") or "").strip(),
        "responsible_user_id": int(payload.get("responsible_user_id") or 0) or None,
        "responsible_name": str(payload.get("responsible_name") or "").strip(),
        "score": score,
        "severity": severity,
        "failures": failures,
        "notes": str(notes if notes is not None else payload.get("notes") or "").strip(),
        "evaluator": str(payload.get("evaluator") or ("IA" if ai_generated else "")).strip(),
        "reviewed_at": reviewed_at,
        "reviewed": True,
        "ai_generated": bool(ai_generated),
        "ai_summary": str(ai_summary or "").strip(),
        "ai_recommendation": str(ai_recommendation or "").strip(),
    }


def upsert_whatsapp_audit_review(conn, review, ai_payload=None):
    now = int(time.time())
    conn.execute(
        """
        insert into whatsapp_audit_reviews
        (audit_key, clinic, lead_id, lead_name, patient_name, patient_phone, pipeline_id, pipeline_name,
         responsible_user_id, responsible_name, score, severity, failures_json, notes, evaluator, reviewed_at,
         created_at, ai_generated, ai_summary, ai_recommendation, ai_payload_json)
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        (
            ...
            review["clinic"],
            review.get("lead_id"),
            review.get("lead_name") or "",
            review.get("patient_name") or "",
            review.get("patient_phone") or "",
            review.get("pipeline_id"),
            review.get("pipeline_name") or "",
            review.get("responsible_user_id"),
            review.get("responsible_name") or "",
            int(review.get("score") or 0),
            review.get("severity") or "pending",
            json.dumps(review.get("failures") or [], ensure_ascii=False),
            review.get("notes") or "",
            review.get("evaluator") or "",
            int(review.get("reviewed_at") or now),
            now,
            1 if review.get("ai_generated") else 0,
            review.get("ai_summary") or "",
            review.get("ai_recommendation") or "",
            json.dumps(ai_payload or {}, ensure_ascii=False),
        ),
    )


def auto_whatsapp_audit_review(payload):
    audit_key = str(payload.get("audit_key") or "").strip()
    lead_id = int(payload.get("lead_id") or 0)
    if not audit_key or not lead_id:
        raise ValueError("Atendimento não informado para avaliação automática.")
    ...
    with db() as conn:
        ...
        if not row:
            raise ValueError("Lead não encontrado na base sincronizada.")

        transcript = lead_message_transcript(conn, lead_id)
        if not transcript:
            raise ValueError("Não encontrei texto de mensagens desse atendimento na base sincronizada do Kommo.")

        contact_info = extract_lead_contact_info(row["raw_json"])
        patient_lookup = build_clinica_patient_lookup(conn)
        patient = find_patient_for_whatsapp_audit(patient_lookup, row["name"], contact_info)
        patient_name = (patient or {}).get("name") or str(payload.get("patient_name") or row["name"] or "").strip()
        patient_phone = (patient or {}).get("phone") or str(payload.get("patient_phone") or (contact_info.get("phones") or [""])[0] or "").strip()
        responsible_name = (
            lead_custom_field_value(row["raw_json"], "Usuário responsável")
            or lead_custom_field_value(row["raw_json"], "Responsável")
            or lead_custom_field_value(row["raw_json"], "Atendente")
            or lead_custom_field_value(row["raw_json"], "Vendedor")
            or str(payload.get("responsible_name") or "").strip()
            or (f"Usuário #{row['responsible_user_id']}" if row["responsible_user_id"] else "")
        )

        rubric_text = json.dumps(WHATSAPP_AUDIT_RUBRIC, ensure_ascii=False)
        transcript_for_ai, transcript_truncated = compact_transcript_for_ai(transcript)
        transcript_text = json.dumps(transcript_for_ai, ensure_ascii=False)
        history_note = (
            f"Histórico textual sincronizado do Kommo: {len(transcript)} mensagens/eventos com texto. "
            "Use todo o histórico abaixo para avaliar continuidade, follow-up, clareza, cordialidade e qualidade do atendimento. "
            "Não penalize tempo de resposta, velocidade no primeiro contato ou prontidão; esse critério foi desativado. "
        )
        if transcript_truncated:
            history_note += "O histórico era maior que o limite técnico; foram mantidos o começo e o fim da conversa, com aviso no ponto omitido. "
        ai_payload = openai_chat_json([
            {
                "role": "system",
                "content": (
                    "Você é uma auditora sênior de atendimento comercial por WhatsApp de uma clínica de estética premium. "
                    "Avalie apenas o atendimento apresentado, com critério profissional e sem inventar fatos ausentes. "
                    "Use exatamente os nomes de falhas da régua recebida. Se não houver evidência suficiente para uma falha, não marque. "
                    "Não avalie tempo de resposta, velocidade ou prontidão no atendimento. "
            ...
            },
            {
                "role": "user",
                "content": (
                    "Régua de avaliação: a nota começa em 100 e cada falha marcada desconta sua penalidade. "
                    f"Use esta régua: {rubric_text}\n\n"
                    "Retorne JSON neste formato: "
                    "{\"failures\":[\"Nome exato da falha\"],\"summary\":\"resumo curto da avaliação\","
                    "\"recommendation\":\"orientação prática para melhorar\","
                    "\"evidence\":[{\"failure\":\"Nome exato da falha\",\"reason\":\"motivo curto\",\"quote\":\"trecho curto se existir\"}]}.\n\n"
                    f"Clínica: {clinic_display_name()}\n"
                    f"Paciente: {patient_name or 'não identificado'}\n"
                    f"Telefone: {patient_phone or 'não identificado'}\n"
                    f"Funil: {row['pipeline_name'] or ''}\n"
                    f"Responsável/atendente no Kommo: {responsible_name or 'não identificado'}\n"
                    f"{history_note}\n"
                    f"Transcrição/eventos em ordem: {transcript_text}"
                ),
            },
        ])

        failures = normalize_whatsapp_audit_failures(ai_payload.get("failures") or [])
        score, severity = whatsapp_audit_score(failures)
        ...
        recommendation = str(ai_payload.get("recommendation") or "").strip()
        notes = summary
        if evidence:
            notes = f"{summary}\n\nEvidências: " + "; ".join(
                str(item.get("reason") or item.get("failure") or "").strip()
                for item in evidence[:5]
                if isinstance(item, dict)
            ).strip()
        review = whatsapp_audit_review_payload(
            {
                "audit_key": audit_key,
                "lead_id": row["id"],
                "lead_name": row["name"] or "",
                "patient_name": patient_name,
                "patient_phone": patient_phone,
        ...
        upsert_whatsapp_audit_review(conn, review, ai_payload=ai_payload)
        return {"ok": True, "review": review}


def build_whatsapp_audit(conn, pipeline_ids, start_ts, end_ts, date_from, date_to, lead_extra_clauses=None, lead_extra_params=None):
    clinic = current_clinic_id()
    where = []
    params = []
    if pipeline_ids:
        placeholders = ",".join("?" for _ in pipeline_ids)
        where.append(f"leads.pipeline_id in ({placeholders})")
        params.extend(pipeline_ids)
    if start_ts and end_ts:
        where.append(
            """
            (
              coalesce(leads.updated_at, leads.created_at, 0) between ? and ?
              or exists (
                select 1 from lead_interaction_events e
                where e.lead_id = leads.id and e.created_at between ? and ?
              )
            )
            """
        )
        params.extend([start_ts, end_ts, start_ts, end_ts])
    for clause in lead_extra_clauses or []:
        if clause:
            where.append(f"({clause})")
    params.extend(lead_extra_params or [])
    where_sql = " and ".join(where) if where else "1 = 1"
    event_start = start_ts or 0
    event_end = end_ts or 4102444799
    rows = conn.execute(
        f"""
        select
            leads.id,
            ...
            leads.raw_json,
            coalesce(pipelines.name, 'Funil ' || leads.pipeline_id) as pipeline_name,
            coalesce(max(lead_interaction_events.created_at), leads.updated_at, leads.created_at) as last_interaction_at,
            count(lead_interaction_events.id) as interaction_count
        from leads
        left join pipelines on pipelines.id = leads.pipeline_id
        left join lead_interaction_events
          on lead_interaction_events.lead_id = leads.id
         and lead_interaction_events.created_at between ? and ?
        where {where_sql}
        group by leads.id
        order by last_interaction_at desc
        limit 300
        """,
        [event_start, event_end, *params],
    ).fetchall()
    patient_lookup = build_clinica_patient_lookup(conn)
    review_rows = conn.execute(
        "select * from whatsapp_audit_reviews where clinic = ?",
        (clinic,),
    ).fetchall()
    reviews = {row["audit_key"]: dict(row) for row in review_rows}
    items = []
    messages_available = False
    for row in rows:
        audit_key = f"{clinic}:lead:{row['id']}"
        samples = lead_message_samples(conn, row["id"])
        if not samples:
            continue
        messages_available = True
        contact_info = extract_lead_contact_info(row["raw_json"])
        patient = find_patient_for_whatsapp_audit(patient_lookup, row["name"], contact_info)
        review = reviews.get(audit_key)
        failures = []
        score = None
        severity = "pending"
        evaluator = ""
        ...
        if review:
            try:
                failures = json.loads(review.get("failures_json") or "[]")
            except (TypeError, ValueError):
                failures = []
            original_failures = list(failures)
            failures = normalize_whatsapp_audit_failures(failures)
            score = int(review.get("score") or 0)
            severity = review.get("severity") or "pending"
            evaluator = review.get("evaluator") or ""
            notes = review.get("notes") or ""
            reviewed_at = review.get("reviewed_at")
            ai_generated = int(review.get("ai_generated") or 0)
            ai_summary = review.get("ai_summary") or ""
            ai_recommendation = review.get("ai_recommendation") or ""
            if failures != original_failures:
                score, severity = whatsapp_audit_score(failures)
                if not failures:
                    ai_summary = "Critério de prontidão desativado; reavalie com IA para uma nova análise completa."
                    ai_recommendation = ""
                    notes = ai_summary
        patient_phone = ""
        patient_email = ""
        ...
        if patient:
            ...
            patient_uuid = patient.get("uuid") or ""
        elif contact_info.get("phones"):
            patient_phone = contact_info["phones"][0]
        responsible_name = (
            lead_custom_field_value(row["raw_json"], "Usuário responsável")
            or lead_custom_field_value(row["raw_json"], "Responsável")
            or lead_custom_field_value(row["raw_json"], "Atendente")
        ...
        items.append({
            ...
            "kommo_lead_url": kommo_lead_url(row["id"]),
            "kommo_search_url": kommo_search_url(patient_phone, row["name"]),
    ...
    scored_items = [item for item in reviewed_items if item.get("score") is not None]
    reviewed_count = len(reviewed_items)
    critical_count = sum(1 for item in reviewed_items if item.get("severity") in ("critical", "grave"))
    average_score = round(sum(float(item.get("score") or 0) for item in scored_items) / len(scored_items), 1) if scored_items else None
    return {
        "date_from": date_from,
        "date_to": date_to,
        "rubric": WHATSAPP_AUDIT_RUBRIC,
        "callers": FOLLOWUP_CALLERS,
        "messages_available": messages_available,
        "totals": {
            "total": len(items),
            "reviewed": reviewed_count,
            "pending": max(0, len(items) - reviewed_count),
...
def save_whatsapp_audit_review(payload):
    audit_key = str(payload.get("audit_key") or "").strip()
    if not audit_key:
        raise ValueError("Atendimento não informado.")
    failures = normalize_whatsapp_audit_failures(payload.get("failures") or [])
    score, severity = whatsapp_audit_score(failures)
    now = int(time.time())
    review = whatsapp_audit_review_payload(
        payload,
    ...
    with db() as conn:
        upsert_whatsapp_audit_review(conn, review)
    return {"ok": True, "score": score, "severity": severity, "failures": failures, "review": review}
...
def query_report_args(params):
    ...
    return {
        ...
        "include_whatsapp_audit": params.get("include_whatsapp_audit", [""])[0] == "1",
    }
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/whatsapp-audit-review":
            with clinic_context(self.request_clinic_id(parsed)):
                if not self.require_clinic_access(parsed):
                    return
                try:
                    payload = self.read_json_body()
                    result = save_whatsapp_audit_review(payload)
                    return json_response(self, result)
                except ValueError as exc:
                    return json_response(self, {"ok": False, "error": str(exc)}, HTTPStatus.BAD_REQUEST)
                except Exception as exc:
                    return json_response(self, {"ok": False, "error": f"Não foi possível salvar a avaliação: {exc}"}, HTTPStatus.BAD_REQUEST)
        if parsed.path == "/api/whatsapp-audit-ai":
            with clinic_context(self.request_clinic_id(parsed)):
                if not self.require_clinic_access(parsed):
                    return
                try:
                    payload = self.read_json_body()
                    result = auto_whatsapp_audit_review(payload)
                    return json_response(self, result)
                except ValueError as exc:
                    return json_response(self, {"ok": False, "error": str(exc)}, HTTPStatus.BAD_REQUEST)
                except Exception as exc:
                    return json_response(self, {"ok": False, "error": f"Não foi possível avaliar com IA: {exc}"}, HTTPStatus.BAD_REQUEST)
